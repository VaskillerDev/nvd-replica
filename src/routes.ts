...
export function resolve(req: IncomingMessage, res: ServerResponse) {
    ...
    const maybeRoute = routes.find(recordedRoute => matchRoutes(incomingRoute, recordedRoute))
    
    if (!maybeRoute) {
        console.error(`route ${incomingRoute.path} on ${incomingRoute.method} not found`)
        return // end
    }
